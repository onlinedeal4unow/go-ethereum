...
type Database struct {
	...
	nodes  map[string]*cachedNode // Data and references relationships of a node
...
func NewDatabase(diskdb ethdb.Database) *Database {
	return &Database{
		diskdb:    diskdb,
		nodes:     map[string]*cachedNode{metaRoot: {}},
...
func (db *Database) insert(owner common.Hash, hash common.Hash, blob []byte, node node) {
	...
	entry.iterateRefs(nil, func(path []byte, child common.Hash) error {
		if c := db.nodes[makeNodeKey(owner, child)]; c != nil {
			c.parents++
		}
		return nil
	})
	...
	if db.oldest == metaRoot {
		...
		db.nodes[db.newest].flushNext, db.newest = key, key
	}
...
func (db *Database) Nodes() []string {
	...
	var keys = make([]string, 0, len(db.nodes))
...
func (db *Database) Reference(owner common.Hash, child common.Hash, parent common.Hash) {
	...
	if db.nodes[parentKey].children == nil {
		db.nodes[parentKey].children = make(map[string]uint16)
	} else if _, ok = db.nodes[parentKey].children[childKey]; ok && parent != (common.Hash{}) {
		return
	}
	node.parents++
	db.nodes[parentKey].children[childKey]++
}
...
func (db *Database) Dereference(root common.Hash, prune bool) error {
	...
	log.Debug("Dereferenced trie from memory database", "nodes", nodes-len(db.nodes), "size", storage-db.nodesSize, "time", common.PrettyDuration(time.Since(start)),
...
func (db *Database) dereference(childOwner common.Hash, childHash common.Hash, parentOwner common.Hash, parentHash common.Hash, prune bool, path []byte, cache map[string]node) error {
	...
	if child.parents == 0 {
		...
		case db.oldest:
			...
			db.nodes[child.flushNext].flushPrev = metaRoot
		case db.newest:
			db.newest = child.flushPrev
			db.nodes[child.flushPrev].flushNext = metaRoot
		default:
			db.nodes[child.flushPrev].flushNext = child.flushNext
			db.nodes[child.flushNext].flushPrev = child.flushPrev
		}
		...
		db.nodesSize -= common.StorageSize(common.HashLength + int(child.size))
	}
	return nil
}
...
func (db *Database) prune(owner common.Hash, hash common.Hash, path []byte, batch ethdb.Batch, cache map[string]node) {
	...
	for key := range db.nodes[metaRoot].children {
		_, root := splitNodeKey(key)

		var paths [][]byte
		if owner != (common.Hash{}) {
			paths = append(paths, keybytesToHex(owner[:]))
		}
...
func (db *Database) live(root node, owner common.Hash, hash common.Hash, paths [][]byte, cache map[string]node) bool {
	// If we reached the end of our path, it must be a hash node
	if len(paths) == 1 && len(paths[0]) == 0 {
		if have, ok := root.(hashNode); ok {
			return common.BytesToHash(have) == hash
		}
		panic(fmt.Sprintf("liveness check terminated on non-hash node: %T %v (%x:%x at %x)", root, root, owner, hash, paths))
	}
	// If we're at a hash node, expand before continuing
	if n, ok := root.(hashNode); ok {
		var key string
		if len(paths) > 1 {
			key = makeNodeKey(common.Hash{}, common.BytesToHash(n))
		} else {
			key = makeNodeKey(owner, common.BytesToHash(n))
		}
		if node := db.nodes[key]; node != nil {
			root = node.node
...
func (db *Database) Cap(limit common.StorageSize) error {
	...
	if db.oldest != metaRoot {
		db.nodes[db.oldest].flushPrev = metaRoot
	}
	...
	log.Debug("Persisted nodes from memory database", "nodes", nodes-len(db.nodes), "size", storage-db.nodesSize, "time", common.PrettyDuration(time.Since(start)),
		"flnodes", db.flushnodes, "flsize", db.flushsize, "fltime", common.PrettyDuration(db.flushtime), "livenodes", len(db.nodes), "livesize", db.nodesSize)
...
func (db *Database) Commit(node common.Hash, report bool) error {
	...
	logger("Persisted trie from memory database", "nodes", nodes-len(db.nodes)+int(db.flushnodes), "size", storage-db.nodesSize+db.flushsize, "time", common.PrettyDuration(time.Since(start)+db.flushtime),
		"gcnodes", db.gcnodes, "gcsize", db.gcsize, "gctime", common.PrettyDuration(db.gctime), "prnodes", db.prunenodes, "prsize", db.prunesize, "prtime", common.PrettyDuration(db.prunetime),
		"linodes", len(db.nodes), "lisize", db.nodesSize)
...
func (db *Database) uncache(owner common.Hash, hash common.Hash) {
	...
	case db.oldest:
		db.oldest = node.flushNext
		db.nodes[node.flushNext].flushPrev = metaRoot
	case db.newest:
		db.newest = node.flushPrev
		db.nodes[node.flushPrev].flushNext = metaRoot
	default:
